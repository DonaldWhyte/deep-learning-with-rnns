...
# TODO: move everything below to future steps
...
# Configurable hyperparameters.
NUM_EPOCHS = 3
STEP_SIZE = BATCH_SIZE * SEQUENCE_LENGTH
...
input_state = np.zeros([BATCH_SIZE, GRU_INTERNAL_SIZE * NUM_HIDDEN_LAYERS])
...
for batch_x, batch_y, current_epoch in batch_sequencer:
    ...
    # Every so often, calculate batch loss and display it to the user.
    step_should_display_accuracy = (
        step % DISPLAY_ACC_EVERY_N_STEPS == 0)
    if step_should_display_accuracy:
        step_batch_loss, step_batch_accuracy = session.run(
            [batch_loss, batch_accuracy],
            feed_dict=step_inputs)
        print(
            f'Epoch {current_epoch}, '
            f'Step {step}, '
            f'Minibatch Loss={step_batch_loss:.4f}, '
            f'Minibatch Accuracy={step_batch_accuracy * 100:.2f}, ')
